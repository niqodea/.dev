...
cmp_nvim_lsp.setup()
local language_files = vim.api.nvim_get_runtime_file('lua/own/modules/lsp-languages/*.lua', true)
for _, language_file in ipairs(language_files) do
    local language = language_file:match(".+/([^/]+)%.lua")
    require('own.modules.lsp-languages.' .. language)(cmp_nvim_lsp)
end
...
cmp.setup {
    ...
    snippet = {
        expand = function(args)
            require('own.core').snippet.snippy.expand_snippet(args.body)
        end,
    },
}
